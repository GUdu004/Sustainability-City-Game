...
class GameService {
    constructor() {
        ...
        this.lastBuildingPosition = { x: 0, y: 0, z: 0 };
    ...
    makeDecision(decisionId, choiceId) {
        ...
        const achievementsUnlocked = achievementService_1.achievementTracker.updateProgress(this.gameState, { type: decision.category, choice: choice });
    ...
    getAvailableBuildingPosition(type) {
        const existingElements = this.gameState.sceneElements;
        const cityRadius = 60;
        let position = { x: 0, y: 0, z: 0 }; // Initialize with default
    ...
    checkEndConditions() {
        ...
        if (environment <= 10 && economy <= 10) {
            ...
            this.gameState.endingDescription = 'Your city has fallen into complete disarray. Both the economy and environment have collapsed, leaving citizens desperate and the city uninhabitable.';
        }
            ...
            const unlockedAchievements = achievementService_1.achievementTracker.getUnlockedAchievements();
    ...
    getAchievements() {
        return {
            unlocked: achievementService_1.achievementTracker.getUnlockedAchievements(),
            progress: achievementService_1.achievementTracker.getProgress()
        };
    }
    resetGame() {
        this.usedDecisionIds.clear();
        achievementService_1.achievementTracker.reset();
        this.gameState = this.initializeGameState();
...
exports.default = gameServiceInstance;
