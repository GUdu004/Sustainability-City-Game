import { GameState, Achievement, AchievementCriteria } from '../types';

// Define achievement criteria and titles
export const ACHIEVEMENT_DEFINITIONS: Record<string, AchievementCriteria> = {
    eco_warrior: {
        id: 'eco_warrior',
        title: '🌱 Eco Warrior',
...
export class AchievementTracker {
    ...
    private statHistory: Array<{ turn: number; stats: any }> = [];
    private decisionHistory: Array<{ turn: number; decisionType: string }> = [];
    ...
    private initializeProgress(): void {
        Object.keys(ACHIEVEMENT_DEFINITIONS).forEach(achievementId => {
            const criteria = ACHIEVEMENT_DEFINITIONS[achievementId].criteria;
    ...
    public updateProgress(gameState: GameState, lastDecision?: any): Achievement[] {
        ...
        Object.entries(ACHIEVEMENT_DEFINITIONS).forEach(([achievementId, achievement]) => {
            ...
            case 'final_balance':
                if (gameState.gameStatus === 'ended') {
                    return criteria.minThreshold !== undefined && criteria.maxThreshold !== undefined &&
                           Object.values(gameState.stats).every(stat => 
                               stat >= criteria.minThreshold! && stat <= criteria.maxThreshold!
                           );
                }
                return false;
    ...
    public getProgressSummary(): Record<string, any> {
